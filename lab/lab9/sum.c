...
int sum_vectorized( int n, int *a )
{
    ...
    __m128i arr = _mm_setzero_si128();
    for( int i = 0; i < n/4*4; i+=4 )
    {
        arr = _mm_add_epi32(arr,_mm_loadu_si128((__m128i*)(a+i)));
    }
    _mm_storeu_si128((__m128i*)b,arr);
    for( int i = 0 ;i < 4; ++i )
        {sum += b[i];}
    for( int i = n/4*4; i < n; i++ )   
        sum += a[i];
...
int sum_vectorized_unrolled( int n, int *a )
{
    ...
    {
        arr = _mm_add_epi32(arr,_mm_loadu_si128((__m128i*)(a+i)));
        arr = _mm_add_epi32(arr,_mm_loadu_si128((__m128i*)(a+i+4)));
        arr = _mm_add_epi32(arr,_mm_loadu_si128((__m128i*)(a+i+8)));
        arr = _mm_add_epi32(arr,_mm_loadu_si128((__m128i*)(a+i+12)));   
    }
    _mm_storeu_si128((__m128i*)b,arr);
    for( int i = 0 ;i < 4; ++i )
        {sum += b[i];}
