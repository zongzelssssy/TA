...
v_add_optimized_adjacent (double *x, double *y, double *z)
{
  #pragma omp parallel
  {
    ...
    int current_thre = omp_get_thread_num();
    for (int i = current_thre; i < ARRAY_SIZE; i = i + nums)
      {z[i] = x[i] + y[i];}
  }
}
...
v_add_optimized_chunks (double *x, double *y, double *z)
{
  ...
  #pragma omp parallel
  {
    ...
    int current_thre = omp_get_thread_num();
    int gap = ARRAY_SIZE / nums;
    Gap = gap;
    for (int i = gap * current_thre; i < gap * (1 + current_thre); i++)
      z[i] = x[i] + y[i];
  }
  for(int i = Gap * end;i < ARRAY_SIZE; i++){z[i] = x[i] + y[i];}
}
